...
### 3.3. Проверка функционала

Для проверки корректности работы всех конструкций (включая вложенность, функции `sqrt` и `len`, а также конкатенацию строк) используются три примера конфигураций из разных предметных областей. Запустите их по очереди, чтобы убедиться, что все вычисления верны и вывод соответствует YAML-формату.

python main.py -i config_web.conf

python main.py -i config_game.conf

python main.py -i config_db.conf
